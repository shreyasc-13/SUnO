...
from recommender import Simulator
...
# TODO: Add plotting style file

###########################################################################
EXP_DIR = '../exps/movielens/'
...
# adobe = conf.adobe
# user_context_length = adobe.user_context_length
...
def run(num_runs):
    ...
    for i in tqdm(range(num_runs)):
        d, eval_rewards = sim.collect_data()
        eval_rs.append(eval_rewards)

        IW, G, rewards = sim.get_weights(d)
        ...
        is_rs.append(estimators.IS(IW, rewards))
        picvs_rs.append(estimators.PICVs(G, rewards))


    return (uno_bins, uno_cdfs), (suno_bins, suno_cdfs), eval_rs, is_rs, picvs_rs
...
def print_results(uno_means, suno_means, eval_rs):
    ...
    print(f'SUnO mean estimate: {np.mean(suno_means):.4f}; Estimate variance: {np.var(suno_means):.4e}')
...
def log_data(uno, suno, eval_rs, is_rs, picvs_rs):
    ...
    with open(f'{EXP_DIR}{exp.name}/is_rewards.pkl', 'wb') as f:
        pickle.dump(is_rs, f)
    with open(f'{EXP_DIR}{exp.name}/picvs_rewards.pkl', 'wb') as f:
        pickle.dump(picvs_rs, f)
...
# Run and log
uno, suno, eval_rs, is_rs, picvs_rs = run(num_runs)
...
if exp.log_data:
    log_data(uno, suno, eval_rs, is_rs, picvs_rs)
